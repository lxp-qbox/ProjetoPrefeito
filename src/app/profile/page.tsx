...
import {
  LogOut, UserCircle2, Save, Globe, Phone, Diamond, MoreHorizontal, MessageSquare, MapPin, Clock, Users, Database, ThumbsUp, UserPlus, Settings as SettingsIcon, Check, Clipboard, DatabaseZap, Lock, CreditCard, Info, ChevronRight, Bell, UserCog, XCircle, Link as LinkIconLucide, ServerOff, FileText, Headphones, LayoutDashboard, Star, Share2, CalendarDays as LucideCalendarIcon, BadgeCheck, Fingerprint, Ticket as TicketIcon, RefreshCw
} from "lucide-react";
import NextImage from 'next/image'; // Ensured NextImage is imported
import { usePathname, useRouter } from "next/navigation";
import { Badge } from "@/components/ui/badge";
import type { UserProfile, FeedPost } from "@/types"; // Ensure ProfileMenuItem and ProfileMenuGroup are not needed if defined locally
...
// Local interface definitions if not shared via types/index.ts
...
export default function ProfilePage() {
  ...
  const profileMenuGroups: ProfileMenuGroup[] = useMemo(() => [
    ...
    {
      isBottomSection: true, // Can be a separate group or styled distinctly in the map
      items: [
    ...
    return [
      {
        ...
        user: { name: currentProfileName, handle: `@${userHandle}`, avatarUrl: currentUser.photoURL || undefined, dataAiHint: "user avatar" },
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentUser?.uid, currentProfileName, userHandle, currentUser?.photoURL]);
  ...
  useEffect(() => {
    ...
    if (validTab) {
      ...
      setActiveTab("visaoGeral"); // Default to visaoGeral
    }
  ...
  const handleSaveProfile = async () => {
    ...
    if (editableBirthDate && isValid(editableBirthDate)) {
      ...
      dataToUpdate.birthDate = null; // Explicitly set to null if not valid or undefined
    }
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [router, handleLogout]); // Added handleLogout to dependencies of handleMenuClick
                    ...
                    {placeholderPosts.map(post => (
                        <PostCard key={post.id} post={{
                            ...
                            user: {
                                ...
                                handle: `@${userHandle}`,
                                avatarUrl: currentUser.photoURL || undefined, // Ensure this is undefined if null for NextImage
                            }
                        }} />
                    ))}
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="pt-2 space-y-1">
                  ...
                  <Select
                    value={editableGender || ""} // Ensure value is never undefined for Select
                ...
                <div className="pt-2 space-y-1">
                  ...
                  <Select
                    value={editableCountry || ""} // Ensure value is never undefined for Select
       ...
       <div className="flex flex-col md:flex-row h-full gap-0 overflow-hidden">
         <nav className="md:w-72 lg:w-80 flex-shrink-0 border-r bg-muted/40 h-full overflow-y-auto p-2 space-y-4">
            {profileMenuGroups.map((group, groupIndex) => (
              <div key={group.groupTitle || `profile-group-${groupIndex}`} className={group.isBottomSection ? "mt-auto pt-4 border-t" : (group.groupTitle ? "" : "pt-2")}>
                {group.groupTitle && (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center ml-auto">
                            ...
                            {(!item.link || (item.link && item.link.startsWith("/profile#"))) && !item.action && !['aparencia', 'suporte', 'sair'].includes(item.id) ? <ChevronRight className="h-4 w-4 text-muted-foreground" /> : null }
                        </div>
                      </Button>
                    );
                  })}
