...
// Simplified interface without groupTitle for this pass
...
export default function ProfilePage() {
  ...
  const placeholderPosts: FeedPost[] = useMemo(() => { 
    ...
    const currentProfileNameForPost = currentUser.profileName || currentUser.displayName || "Usuário";
    const userHandleForPost = currentUser.email?.split('@')[0]?.toLowerCase() || "usuario";

    return [
    {
      id: "post1-profile",
      userId: currentUser.uid,
      user: { name: currentProfileNameForPost, handle: `@${userHandleForPost}`, avatarUrl: currentUser.photoURL || undefined, dataAiHint: "user avatar" },
  ...
  const profileMenuGroups: ProfileMenuGroup[] = useMemo(() => [
    {
      items: [
        { id: "visaoGeral", title: "Visão Geral", icon: UserCircle2 },
  ...
  useEffect(() => {
    ...
    if (currentItem) {
      ...
      const defaultProfileTab = allItems.find(item => item.link === pathname || (item.id && `/profile#${item.id}` === pathname));
            ...
            <div className="px-6 pb-6 relative">
              <div className="flex flex-col items-center -mt-16 sm:-mt-20 relative z-10"> 
                ...
                <div className="mt-4 text-center">
                    ...
                    <p className="mt-1 text-sm text-foreground/90 px-4 md:px-0 max-w-md mx-auto break-words">{currentUser.bio || "UX/UI Designer, 4+ years of experience"}</p>
                </div>
