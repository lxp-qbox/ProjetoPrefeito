    ...
    if (currentUser) {
      ...
      if (currentUser.birthDate) {
        try {
          ...
          } else if ((currentUser.birthDate as any)?.toDate) { // Check for Firestore Timestamp
            parsedDate = (currentUser.birthDate as any).toDate();
          } else if (currentUser.birthDate instanceof Date) { // Check if already a Date object
  ...
  const joinedDateFormatted = useMemo(() => {
    if (currentUser?.createdAt && (currentUser.createdAt as any).toDate) {
      try {
        ...
        try {
            const date = parseISO(currentUser.createdAt);
            if (isValid(date)) {
    ...
    <ProtectedPage>
      <div className="flex flex-col h-full">
        ...
        <div className="bg-card rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="px-4 pb-6 sm:px-6 text-center -mt-14 relative">
            <Avatar className="h-28 w-28 mx-auto ring-4 ring-background shadow-lg">
            ...
            <p className="text-base text-muted-foreground mt-1 max-w-md mx-auto">{userShortBio}</p>
            <div className="flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-sm text-muted-foreground mt-2">
              <span className="flex items-center"><MapPin className="mr-1 h-4 w-4" />{userLocation}</span>
              <span className="text-muted-foreground/50">|</span>
              <span className="flex items-center"><LucideCalendarIcon className="mr-1 h-4 w-4" />Entrou {joinedDateFormatted}</span>
            </div>
            <div className="mt-6 flex justify-center gap-3">
              <Button className="bg-pink-500 hover:bg-pink-600 text-white rounded-full px-6 py-2.5 text-sm font-semibold">
                Inscrever-se
              </Button>
              <Button variant="outline" className="rounded-full px-6 py-2.5 text-sm font-semibold">
                Editar perfil
              </Button>
            </div>
          </div>
        ...
        <div className="flex-grow flex flex-col md:flex-row gap-6 mt-6 overflow-hidden">
          ...
          <nav className="md:w-72 lg:w-80 flex-shrink-0 h-full overflow-y-auto bg-card shadow-md rounded-lg p-4 space-y-4">
                    ...
                    return (
                      <Button
                        ...
                        className={cn(
                          ...
                          !isActive && "text-card-foreground hover:bg-muted hover:text-card-foreground" // Keep text color same on hover for inactive
                        )}
