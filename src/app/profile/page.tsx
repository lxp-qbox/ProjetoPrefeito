...
import NextImage from 'next/image'; // Correct import for NextImage
...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    if (currentItem) {
      ...
      const defaultProfileTab = allItems.find(item => item.link === pathname || (item.link && `/profile#${item.id}` === pathname));
      ...
      case 'visaoGeral':
        return (
           <div className="w-full bg-card"> {/* Ensures white background for this section */}
            {/* Banner */}
            <div className="h-40 bg-gradient-to-r from-primary/30 to-primary/10 relative"> {/* Changed accent to primary/10 */}
